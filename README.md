...
.agents/                               # Generated by .ai/sync.sh (Antigravity)
├── rules/                             # Agent rules (@mentionable)
└── workflows/                         # Prompts as /invocable workflows
...
Everything goes in a single always-loaded file. Agents must be compiled into a compact version. For these tools, **Spec Driven Development (SDD) is strongly recommended** — Feature Specs centralize context and compensate for the lack of on-demand agents. See `docs/specs/FEAT-TEMPLATE.md` and the `/feature-spec` prompt.
...
| Capability | Claude Code | OpenCode | Antigravity | Codex | Cursor | Windsurf | Gemini | Copilot |
|---|---|---|---|---|---|---|---|---|
| Sub-agents | Yes (Task tool) | Yes (spawn) | No | No | No | No | No | No |
| Orchestrator | No (auto routing) | Yes (orchestrator.md) | No | No | No | No | No | No |
| Model routing | No (1 model/session) | Yes (per agent) | No | No | Yes (per rule) | No | No | No |
| On-demand agents | Yes (.claude/agents/) | Yes (.opencode/agents/) | Yes (.agents/rules/ @mention) | No | No | No | No | No |
| Workflows | No (prompts via file) | No | Yes (/workflow-name) | No | No | No | No | No |
| Agent teams | Experimental | No | No | No | No | No | No | No |
| MCPs | Yes | Yes | Yes | Yes | Yes | Partial | Partial | Partial |
