...
ensure_gitignore() {
    ...
    local marker="# Generated by .ai/sync.sh"

    # Entries that sync.sh-generated files need
    local -a entries=(
        ".claude/agents/"
        ".claude/rules/"
        ".claude/skills"
    ...
    if $DRY_RUN; then
        log_dry "Would ensure .gitignore entries"
        return
    fi

    # Create .gitignore if it doesn't exist
    if [[ ! -f "$gitignore" ]]; then
        printf '%s\n' "$marker" "${entries[@]}" > "$gitignore"
        log_info ".gitignore (created with sync entries)"
        return
    fi

    # If marker already exists, entries were added before
    if grep -qF "$marker" "$gitignore" 2>/dev/null; then
        log_info ".gitignore (sync entries already present)"
        return
    fi
    ...
    log_info ".gitignore (sync entries added)"
}
