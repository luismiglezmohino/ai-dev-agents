...
```bash
# macOS / Linux
cp -r ai-dev-agents/{.ai,.claudeignore,AGENTS.md,docs} tu-proyecto/

# Windows (PowerShell)
Copy-Item -Recurse ai-dev-agents\.ai, ai-dev-agents\docs, ai-dev-agents\AGENTS.md, ai-dev-agents\.claudeignore tu-proyecto\
```
...
El template incluye un workflow de GitHub Actions (`.github/workflows/ai-agents-validate.yml`) que ejecuta `sync.sh + test.sh` en cada PR y push a main. Si adoptaste el template via "Use this template", ya lo tienes. Valida la estructura de agentes, frontmatter y ficheros generados automáticamente.

Si no lo quieres, borra `.github/workflows/ai-agents-validate.yml`. Si ya tienes tu propio CI, el workflow del template no entra en conflicto — se ejecuta independientemente.
